-- from https://github.com/mattorb/dotfiles/blob/3fc63f22005bf5298acbcdc8dfa5df89eb824693/hammerspoon/drawonscreen/init.lua
local cache = {}
...
module.startAnnotating = function()
	...
	cache.canvas:canvasMouseEvents(false, false, false, true):mouseCallback(function(_, _, _, _, _) end)
...
module.stopAnnotating = function()
	if cache.canvas then
		cache.canvas:canvasMouseEvents(false, false, false, false):mouseCallback(nil)
	end
...
local setup = function()
	if not cache.canvas then
		cache.canvas = hs.canvas.new({ x = 0, y = 0, w = 0, h = 0 }):level(hs.canvas.windowLevels.overlay):behavior({
	...
	cache.canvas:size(screen:frame()):frame(screen:frame()):show()
